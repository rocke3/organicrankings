    ...
    <client-only>
      <div class="row">
          ...
          <div class="border rounded bg-white p-4 text-center border-primary shadow-primary mb-4"
            ...
            <div class="text-start">
              <table class="table text-success">
                <tr>
                  <td>HTML tools</td>
                  <td class="text-end">{{ data.subscription.sb_html ?? 0 }} out of {{ data.subscription.sp_html }}
                  </td>
                </tr>
                <tr>
                  <td>CSS tools</td>
                  <td class="text-end">{{ data.subscription.sb_css ?? 0 }} out of {{ data.subscription.sp_css }}
                  </td>
                </tr>
                <tr>
                  <td>JS tools</td>
                  <td class="text-end">{{ data.subscription.sb_js ?? 0 }} out of {{ data.subscription.sp_js }}
                  </td>
                </tr>
                <tr>
                  <td>Image</td>
                  <td class="text-end">{{ data.subscription.sb_img ?? 0 }} out of {{ data.subscription.sp_image }}
                  </td>
                </tr>
                <tr>
                  <td>Page Speed</td>
                  <td class="text-end">{{ data.subscription.sb_pageSpeed ?? 0 }} out of {{
        ...
        <div class="col-xl-3 col-md-6" v-for="plan in plans">
          <div class="card mb-5 planBox active"
            v-if="(data.subscription.sb_plan == plan.sp_id && data.subscription.sb_active)">
            ...
            <div class="card-footer p-3">
              ...
              <table class="table text-success font-weight-bolder text-sm">
                <tr>
                  <td>HTML tools</td>
                  <td class="text-end"><span class="text-dark p-0">{{ data.subscription.sb_html }}</span> of {{
                      plan.sp_html
                  }}</td>
                </tr>
                <tr>
                  <td>CSS tools</td>
                  <td class="text-end"><span class="text-dark p-0">{{ data.subscription.sb_css }}</span> of {{
                      plan.sp_css
                  }}</td>
                </tr>
                <tr>
                  <td>JS tools</td>
                  <td class="text-end"><span class="text-dark p-0">{{ data.subscription.sb_js }}</span> of {{
                      plan.sp_js
                  }}</td>
                </tr>
                <tr>
                  <td>Image</td>
                  <td class="text-end"><span class="text-dark p-0">{{ data.subscription.sb_image }}</span> of {{
                      plan.sp_image
                  }}</td>
                </tr>
                <tr>
                  <td>Page Speed</td>
                  <td class="text-end"><span class="text-dark p-0">{{ data.subscription.sb_pageSpeed }}</span> of {{
                      plan.sp_pageSpeed
                  }}</td>
                </tr>
              </table>
          ...
          <div class="card mb-5 planBox" v-else>
            ...
            <div class="card-footer p-3">
              ...
              <div class="text-center mt-4">
                <button v-if="data.subscription.sb_plan == plan.sp_id && subActive"
                  class="text-bold btn btn-outline-primary font-weight-bolder" disabled>
                  Currently Using
                </button>
                <button class="btn btn-primary"
                  @click="subscribe(plan.sp_id, plan.sp_stripePrice, subActive); plan.processing = 1"
                  :disabled="(plan.sp_id < data.subscription.sb_plan && subActive) ? true : false" v-else>
