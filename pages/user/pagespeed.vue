...
useHead({ title: "Page Speed - Organic Rankings" });
let result = ref("")
let show = ref("mobile")
let website = ref("")
let processing = ref(false)
let error = ref({ error: false, message: '' })
...
function resultView($device) {
  show.value = $device;
  if ($device == "mobile") {
    document.querySelector('[aria-labelledby="mobile_tab"]').style.display = "block";
    document.querySelector('[aria-labelledby="desktop_tab"]').style.display = "none";
  } else {
    document.querySelector('[aria-labelledby="mobile_tab"]').style.display = "none";
    document.querySelector('[aria-labelledby="desktop_tab"]').style.display = "block";
  }
}
...
<template>
  <div>
    <ElementsBsCard formTitle="Page Speed Analysis" titleClass="ps-3">
      ...
      <ul class="nav nav-tabs" v-if="result">
        <li class="nav-item">
          <a class="nav-link" :class="show == 'mobile' ? 'active' : ''" href="#"
            @click="resultView('mobile')">Mobile</a>
        </li>
        <li class="nav-item">
          <a class="nav-link" :class="show == 'mobile' ? '' : 'active'" href="#"
      ...
      <div v-if="error.error" class="text-center">
        <div class="d-flex justify-content-center mb-2">
          <div class="alert alert-danger text-white text-bold px-5" v-html="error.message"></div>
        </div>
...
.lh-warnings {
  ...
  padding: var(--default-padding) var(--default-padding) var(--default-padding) calc(var(--audit-description-padding-left));
...
.lh-expandable-details[open] summary,
details.lh-clump>summary {
  border-bottom: 1px solid var(--report-border-color-secondary);
}
...
details[open] .lh-clump-toggletext--show {
  display: none;
}

details[open] .lh-clump-toggletext--hide {
  display: block;
}
...
@keyframes fadeInTooltip {
  ...
  100% {
    opacity: 1;
    filter: drop-shadow(1px 0px 1px #aaa) drop-shadow(0px 2px 4px hsla(206, 6%, 25%, 0.15));
    pointer-events: auto;
  }
}
